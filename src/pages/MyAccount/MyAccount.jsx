  ...
  const formItems = [
    {
      ...
      labelOne: "نام کاربری یا آدرس ایمیل",
      labelTwo: "گذرواژه",
      forgetPassword: "گذرواژه خود را فراموش کرده اید؟",
      submitBtn: "ورود به نگارشارپ",
    },
    {
      id: 2,
      title: "ثبت نام در نگار شاپ",
      labelOne: "آدرس ایمیل",
      labelTwo: "گذرواژه",
      security:
        "اطلاعات شخصی شما نزد ما نگهداری میشود و به هیچ عنوان سوءاستفاده نخواهد شد.",
      submitBtn: " ثبت نام در نگارشارپ",
    },
  ];
        ...
        <div className="flex flex-col items-center mt-6 lg:mt-12">
          ...
          <div className="w-full sm:w-[500px] bg-white overflow-hidden rounded-[20px] mt-8">
            <ul className="flex justify-evenly items-center">
              <li
                to="#login"
                onClick={() => setStatusAccount(true)}
                className={`${
                  statusAccount ? "bg-white" : "bg-[#f5f5f5]"
                } w-1/2 p-4 rounded-bl-[20px]`}
              >
                <button
                  className={`flex items-center ${
                    statusAccount ? "text-primary" : "text-[#aaa]"
                  }`}
                >
              ...
              <li
                to="#register"
                onClick={() => setStatusAccount(false)}
                className={`${
                  !statusAccount ? "bg-white" : "bg-[#F5F5F5]"
                } w-1/2 p-4 rounded-bl-[20px]`}
              >
            ...
            {statusAccount ? (
              <FromItem {...formItems[0]} />
            ) : (
              <FromItem {...formItems[1]} />
            )}
          </div>
        </div>
      </div>
